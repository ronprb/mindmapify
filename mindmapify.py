...
def generate_mind_map_md(summarized_text):
    client = anthropic.Anthropic(api_key="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")
    ...
    if isinstance(message.content, list) and len(message.content) > 0:
        mind_map = message.content[0].text.strip()
    else:
        raise ValueError(f"Unable to extract mind map from the message response. Response: {message}")
